...
export const BeeaChat: React.FC<BeeaChatProps> = ({ isOpen, onClose, userId }) => {
    ...
    const safePadding = useSafePadding({ basePadding: 3.5 });
    const footerPadding = isKeyboardOpen ? 0 : safePadding;
    ...
    // Audio functionality removed to conserve API quota
    ...
    return (
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-0 sm:p-6 animate-in fade-in duration-200">
            ...
            <div className="relative w-full max-w-lg h-full sm:h-[600px] bg-[#0B0F1A] border-t sm:border border-white/10 rounded-none sm:rounded-[3rem] shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom duration-300">
                {/* Header */}
                <header className="px-6 py-4 bg-gradient-to-r from-primary/10 to-transparent border-b border-white/5 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <div className="w-14 h-14 rounded-2xl bg-primary/20 flex items-center justify-center border border-primary/40 overflow-hidden shadow-[0_0_15px_rgba(250,204,21,0.4)]">
                                <img
                            ...
                            <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-[#0B0F1A] shadow-lg"></div>
                        </div>
                        <div>
                            <h3 className="text-base font-black uppercase tracking-tighter text-white leading-none">Beea</h3>
                            <p className="text-[9px] font-bold text-primary uppercase tracking-widest mt-0.5 italic">Assistente Polinizadora</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <button
                            onClick={clearChat}
                            className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-white/40 hover:text-red-400 transition-colors"
                            title="Limpar Conversa"
                        >
                            <span className="material-symbols-outlined text-lg">delete_sweep</span>
                        </button>
                        <button onClick={onClose} className="w-9 h-9 rounded-full bg-white/5 flex items-center justify-center text-white/40 hover:text-white transition-colors">
                ...
                {/* Input Area */}
                <footer
                    className="p-6 bg-white/[0.04] border-t border-white/5 backdrop-blur-xl transition-all duration-300"
